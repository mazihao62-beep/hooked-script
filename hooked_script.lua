...
print("[上钩了] v1.3 OK")
local S={AA=false,SA=false,SPD=false,SPDV=24,FLY=false,FLYV=50,AutoHeal=false,ESP=false,ESPR=200};local KB={W="RightShift"};local WN,CT=nil,{};local flyState=false
local function gHRP(pl)return pl and pl.Character and pl.Character:FindFirstChild("HumanoidRootPart")end
local function gN(r)
    local c=LP.Character;local hrp=c and c:FindFirstChild("HumanoidRootPart");if not hrp then return nil,999 end
    local p=hrp.Position;local b,bD=nil,r or 200
...
local function doAim()
    local t,d=gN(200);if not t then return end
    local hp=gHRP(t);if not hp then return end
    local c=LP.Character;local my=c and c:FindFirstChild("HumanoidRootPart")
    if not my then return end
    if S.AA then CA.CFrame=CFrame.lookAt(CA.CFrame.Position,hp.Position)end
    local dir=(hp.Position-my.Position).Unit
    pcall(function()HF:FireServer({hookId=tostring(os.clock()*1000),batchId="auto",hookCount=1,startPosition=my.Position,direction=dir,distance=(hp.Position-my.Position).Magnitude,hookTravelTime=0.2,hookBackSpeed=60,fireTime=os.clock()})end)
end
local function enableFly()
    local c=LP.Character;local hrp=c and c:FindFirstChild("HumanoidRootPart");if not hrp then return end
    local h=c:FindFirstChildOfClass("Humanoid");if h then h.PlatformStand=true end;flyState=true
    spawn(function()while flyState do
        local c2=LP.Character;local hrp2=c2 and c2:FindFirstChild("HumanoidRootPart");if not hrp2 then break end
        local spd=S.FLYV;local cf=CA.CFrame;local fwd=cf.LookVector*Vector3.new(1,0,1).Unit;local rgt=cf.RightVector*Vector3.new(1,0,1).Unit;local mv=Vector3.new()
        ...
        if mv.Magnitude>0 then mv=mv.Unit*spd*0.05;hrp2.CFrame=hrp2.CFrame+mv end
        wait(0.03)end;local c3=LP.Character;local h3=c3 and c3:FindFirstChildOfClass("Humanoid");if h3 then h3.PlatformStand=false end end)
end
local function disableFly()flyState=false;local c=LP.Character;local h=c and c:FindFirstChildOfClass("Humanoid");if h then h.PlatformStand=false end end
local ESP={}
local function cESP()for _,d in pairs(ESP)do pcall(function()d:Remove()end)end;ESP={}end
local function uESP()
    if not S.ESP then cESP();return end
    local c=LP.Character;local hrp=c and c:FindFirstChild("HumanoidRootPart");local pos=hrp and hrp.Position;local seen={}
    for _,pl in ipairs(P:GetPlayers())do if pl~=LP then
        local hp=gHRP(pl)
        if hp then local d=pos and(hp.Position-pos).Magnitude or 999
            if d<=S.ESPR then
                if not ESP[pl]then
                    local n=Drawing.new("Text");n.Size=16;n.Center=true;n.Outline=true;n.Color=Color3.fromRGB(255,50,50);n.Font=2
                    local b=Drawing.new("Square");b.Thickness=2;b.Color=Color3.fromRGB(255,50,50)
                    local l=Drawing.new("Line");l.Thickness=1;l.Color=Color3.fromRGB(255,255,255,100)
                    ESP[pl]={T=n,B=b,L=l}
                end
                local e=ESP[pl];local sp,on=CA:WorldToViewportPoint(hp.Position)
                if on then e.T.Position=Vector2.new(sp.X,sp.Y-30);e.T.Text=pl.Name.." ["..math.floor(d).."m]";e.T.Visible=true;e.B.Position=Vector2.new(sp.X-25,sp.Y-40);e.B.Size=Vector2.new(50,80);e.B.Visible=true;e.L.From=Vector2.new(CA.ViewportSize.X/2,CA.ViewportSize.Y);e.L.To=Vector2.new(sp.X,sp.Y);e.L.Visible=true;seen[pl]=true else e.T.Visible=false;e.B.Visible=false;e.L.Visible=false end end end end end
    for pl,d in pairs(ESP)do if not seen[pl]then d.T.Visible=false;d.B.Visible=false;d.L.Visible=false end end
end
WN=WI:CreateWindow({Title="上钩了! v1.3",Author="b站英吉利超入_",Icon="solar:hook-bold",Size=UDim2.fromOffset(750,520),ToggleKey=Enum.KeyCode.RightShift,Folder="hooked-script",Acrylic=true,Resizable=false,ScrollBarEnabled=true,HideSearchBar=true,
    OnClose=function()cESP();S.FLY=false;disableFly();for _,ct in pairs(CT)do if ct and type(ct.Set)=="function"then pcall(function()ct:Set(false)end)end end end})
...
CT.AA=t1:Toggle({Flag="AA",Title="自动瞄准",Value=false,Callback=function(v)S.AA=v end})
CT.SA=t1:Toggle({Flag="SA",Title="静默瞄准(不可见)",Value=false,Callback=function(v)S.SA=v end})
t1:Divider()
...
t1:Divider()
CT.HEAL=t1:Toggle({Flag="HEAL",Title="自动拾取血包",Value=false,Callback=function(v)S.AutoHeal=v end})
local t2=WN:Tab({Title="透视",Icon="solar:eye-bold"})
CT.ESP=t2:Toggle({Flag="ESP",Title="玩家透视(方框+名字)",Value=false,Callback=function(v)S.ESP=v;if not v then cESP()end end})
...
t7:Paragraph({Title="上钩了! v1.3"});t7:Divider();t7:Paragraph({Title="作者",Desc="b站英吉利超入_"})
t7:Paragraph({Title="功能",Desc="TEvent直连HookFire+静默瞄准+ESP+飞行+加速"})
...
spawn(function()
    while true do
        ...
        pcall(uESP)
        local now=tick()
        if now-last>2 then last=now;if sp5 then pcall(function()sp5:SetTitle("在线玩家: "..#P:GetPlayers())end)end end
        wait(0.05)
    end
end)